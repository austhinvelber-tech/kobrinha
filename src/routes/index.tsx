...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      { title: "SNAKE.EXE — Snake retrô Game Boy, jogue grátis" },
      {
        name: "description",
        content:
          "Jogue Snake clássico em estilo Game Boy direto no navegador: teclado ou celular, pontuação, recorde salvo e som retrô. 100% grátis.",
      },
      { property: "og:title", content: "SNAKE.EXE — Snake retrô Game Boy" },
      {
        property: "og:description",
        content:
          "Snake clássico em estilo Game Boy: jogue no navegador, bata o recorde e compartilhe.",
      },
...
function SnakeGame() {
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center px-4 py-8">
      {/* HUD */}
      <div className="mx-auto mb-3 flex w-full max-w-[340px] items-end justify-between border-b-2 border-lcd-ink pb-2">
        <div>
          <p className="text-[6px] opacity-60">SCORE</p>
          <p className="font-crt text-2xl leading-none">{String(score).padStart(4, "0")}</p>
        </div>
        <p className="text-sm tracking-tight">SNAKE.EXE</p>
      ...
      {/* Tabuleiro */}
      <div
        className="relative mx-auto w-full max-w-[340px] border-4 border-lcd-ink bg-lcd-bg shadow-[6px_6px_0_0_var(--color-lcd-ink)]"
      ...
      {/* D-pad para celular */}
      <div className="mx-auto mt-6 grid w-full max-w-[180px] grid-cols-3 gap-2">
        <div />
      ...
      <p className="mt-8 text-[6px] opacity-40">SYSTEM V.2026.0.1</p>
    </main>
  );
}
